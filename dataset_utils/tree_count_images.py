    ...
    if not max_depth or depth <= max_depth:
        for item in os.listdir(dir_path):
            ...
            if os.path.isdir(item_path):
                ...
                amount_of_jpgs = get_system_output(
                    f"find {item_path} -type f -iname '*.jpg' | wc -l"
                )
                # fileCountString = f"         (Ref: {amount_of_ref} Probe: {amount_of_probe})"
                # subjects = get_system_output(f"ls -1 {amount_of_pngs}| wc -l")
                fileCountString = f"         (amount_of_jpgs: {amount_of_jpgs})"
...
# Example usage:
tree("./cleaned_source_images_for_generating_morphs/", max_depth=2)
