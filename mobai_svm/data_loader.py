...
class data_loader:
    def __init__(
        ...
        self.sub_folders = (
            ...
            else ["1_male_source", "2_female_source"]
        )
        self.feat_shape = feat_shape
        self.save_ds_path = os.path.join(
            os.path.dirname(os.path.dirname(self.morphed_path)), "data_full"
        )
    ...
    def construct_pipeline(self):
        ...
        for ds_i in data_sets:
            for folder_i in self.sub_folders:
                ...
                for i, morph_path in enumerate(glob.glob(morph_paths + "/ref_*.txt")):
                    ...
                    probe_path1 = os.path.join(
                        os.path.dirname(morph_path), "probe_" + probe_id1 + ".txt"
                    )
                    probe_path2 = os.path.join(
                        os.path.dirname(morph_path), "probe_" + probe_id2 + ".txt"
                    )
            ...
            for folder_i in self.sub_folders:
                ...
                for i, ref_path in enumerate(glob.glob(ref_paths + "/ref_*.txt")):
                    ...
                    probe_path = os.path.join(
                        os.path.dirname(ref_path), "probe_" + probe_id + ".txt"
                    )
        ...
        if self.save_path_flag:
            ...
            with open(
                os.path.join(self.save_ds_path, self.flag + "_full_paths.txt"), "w"
            ) as file:
                for row in full_paths:
                    file.write(" ".join([str(item) for item in row]))
                    file.write("\n")
            with open(
                os.path.join(self.save_ds_path, self.flag + "_full_labels.txt"), "w"
            ) as file:
    ...
    def load_full_data(self):
        return pickle.load(
            open(os.path.join(self.save_ds_path, self.flag + ".pkl"), "rb")
        )
    ...
    def get_full_data(self, save_flag=False, load_from_file=True):
        if load_from_file and os.path.exists(
            os.path.join(self.save_ds_path, self.flag + ".pkl")
        ):
            x_full, y_full = self.load_full_data()
            if "test" in self.flag:
                meta_data = pickle.load(
                    open(
                        os.path.join(self.save_ds_path, "meta_" + self.flag + ".pkl"),
                        "rb",
                    )
                )
            else:
                ...
                for i, pair_path in enumerate(ds_i):
                    ...
                    if "test" in self.flag:
                        ...
                        meta_data["dataset"].append(
                            pair_path[0].split("Feature_")[-1].split("/")[1]
                        )
            ...
            if save_flag:
                ...
                if "test" in self.flag:
                    pickle.dump(
                        ...
                        open(
                            os.path.join(
                                self.save_ds_path, "meta_" + self.flag + ".pkl"
                            ),
                            "wb",
                        ),
                    )
