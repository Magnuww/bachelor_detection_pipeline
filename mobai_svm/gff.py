...
argparser.add_argument("--list", type=str, help="List of feature files to find matching pngs")
argparser.add_argument("--out", type=str, help="Output file to write the list of pngs to")
        ...
        if len(rsplt) > 1:
            ...
            if rsplt[1] == "AGE":
                ...
                if file.endswith("1.jpg"):
                    copy_image(root + "/", file, args.out + "/" + "/".join(rsplt[:-1]) + "/", False)
                if file.endswith("2.jpg"):
                    copy_image(root + "/", file, args.out + "/" + "/".join(rsplt[:-1]) + "/", True)
            else:
