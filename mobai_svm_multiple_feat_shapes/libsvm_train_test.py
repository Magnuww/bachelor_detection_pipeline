...
def plot_APCER_BPCER(p_vals, labels, step=0.01, logscale=True, label="", plot_flag=True):
    apcer = []
    bpcer = []
    thres = []
...
def tune_svm(strInputBonafideFeaturesFolders, strInputAttacksFeaturesFolders, strSaveResultsFolder):
    ...
    setups = np.array(np.meshgrid(*tune_params.values())).T.reshape(-1, len(tune_params))
    params = list(tune_params.keys())
    
    np.random.seed(666)
    train_loader = data_loader(strInputAttacksFeaturesFolders, strInputBonafideFeaturesFolders, flag='1_training_set')    
    train_x, train_y, _ = train_loader.get_full_data(save_flag=True, load_from_file=True)      # x=[49xNx512], y=[N,]

    # Split train set into train_val: 
    train_indxs = np.random.choice(len(train_y), int(len(train_y)*train_val_split), replace=False)
    ...
    # Class weights: 
    weight_0 = 1
    weight_1 = np.round((len(train_y) - sum(np.array(train_y) == 1)) / sum(np.array(train_y) == 1), 5)
    ...
    if os.path.exists(os.path.join(strSaveResultsFolder, run_name + "results.pkl")):
        results = pickle.load(open(os.path.join(strSaveResultsFolder, run_name + "results.pkl"), "rb"))
        setup_df = pickle.load(open(os.path.join(strSaveResultsFolder, run_name + "params.pkl"), "rb"))
        train_indxs = pickle.load(open(os.path.join(strSaveResultsFolder, run_name + "train_indxs.pkl"), "rb"))
    ...
    for i, setup_array_i in enumerate(setups):
        ...
        if "weight_imbalance" in setup_i: 
            if setup_i["weight_imbalance"] == '1': 
                param_str += f' -w0 {weight_0} -w1 {weight_1}'      # .format(weight_0, weight_1)
                ...
                mp_indxs = np.sort(np.random.choice(mp_indxs, len(bf_indxs), replace=False))
                us_indxs = np.concatenate([bf_indxs, mp_indxs])
                train_y_us = [train_y_us[ind] for ind in us_indxs]
                train_x_us = [[train_x_us[i][j] for j in us_indxs] for i in range(len(train_x))]
                ...
                train_x_us = [[train_x_us[i][j] for j in us_indxs] for i in range(len(train_x))]
                assert np.sum(np.array(train_y_us) == 1) / len(train_y_us) == 0.5
                print(f"Ds Size {len(train_y_us)}, num_morph {np.sum(np.array(train_y_us) == 1)}")
                train_x = train_x_us
                train_y = train_y_us                
        ...
        apcr, bpcr, _ = plot_APCER_BPCER(p_vals, np.array(val_y), step=0.01, plot_flag=False)
        results["APCR"].append(apcr)
        results["BPCR"].append(bpcr)

        # Compute accuracies for different thresholds: 
        thresholds = np.arange(0.05, 1, 0.05)
        acc_t_i = [1 - np.abs(np.array(val_y) - (p_vals.mean(0) >= t_i).astype(int)).sum() / len(val_y) for t_i in thresholds]
        ...
        pickle.dump(results, open(os.path.join(strSaveResultsFolder, run_name + "results.pkl"), "wb"))
        setup_df = pd.DataFrame(setups, columns=params)
        pickle.dump(setup_df, open(os.path.join(strSaveResultsFolder, run_name + "params.pkl"), "wb"))
        pickle.dump(train_indxs, open(os.path.join(strSaveResultsFolder, run_name + "train_indxs.pkl"), "wb"))



def train_svm(strInputBonafideFeaturesFolders, strInputAttacksFeaturesFolders, strSavingModelFilePath, feature_shape, param_str=None, oversampled=False):
    if not os.path.exists(strSavingModelFilePath):
        os.makedirs(strSavingModelFilePath)
    train_loader = data_loader(strInputAttacksFeaturesFolders, strInputBonafideFeaturesFolders, feat_shape=feature_shape, flag='2_dev_set',
                               sub_folders=None)    
    ...
    for i, x_i in enumerate(train_x): 
        if not os.path.exists(strSavingModelFilePath + 'model_' + str(i) + '.model'):
            ...
            # Save the model to a file
            print(f'Finished Loop {i} og {len(train_x)}')
            svmutil.svm_save_model(strSavingModelFilePath + 'model_' + str(i) + '.model', model_i)


def test_svm(strInputBonafideFeaturesFolders, strInputAttacksFeaturesFolders, strSavingModelFilePath, feature_shape, load_preds=False):
    test_loader = data_loader(strInputAttacksFeaturesFolders, strInputBonafideFeaturesFolders, feat_shape=feature_shape, flag='3_test_set')
